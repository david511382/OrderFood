...
function init() {
    ...
    for (let i = 0; i < menuData.length; i++) {
        ...
        let smallPriceStr = itemData.Price.toString(10);
        let largePriceStr = (itemData.Price + plusPrice).toString(10);
...
fs.push(function() {
    ...
    <select id="`+nameSelect+`">
        <option value ="`+smallPriceStr+`">小</option> <option value = "`+largePriceStr+`" > 大 </option></select>
    ...
    數量<input id="`+nameAmount+`" type="text" name="ammount" value="1" onfocus="this.select()" onkeypress="onOrderChang(`+nameSelect+`,`+nameRice+`,`+nameCheese+`,`+nameAmount+`,`+namePrice+`); if (window.event.keyCode==13) return false;">個`
        ...
        itemInfoDiv.innerHTML += itemDiv.id + spaces + "小" + smallPriceStr + " 大" + largePriceStr + "</br > ";
